#!/usr/bin/env node
/**
 * OpenAI Image Generation Script for AutoNateAI - Shop Page
 *
 * Generates social preview images using GPT Image 1
 *
 * Usage:
 *   node generate-og-shop.js
 *
 * Environment:
 *   OPENAI_API_KEY - Your OpenAI API key
 *
 * Output:
 *   courses/assets/og-shop.png
 */
...
const CONFIG = {
  ...
  prompt: `Create a stunning social media preview image for "AutoNateAI" shop page - advanced search query packs with tutorials.
...
- Subtitle text: "Advanced Search Packs + Tutorials"
- Boolean search operators (AND, OR, NOT) floating as glowing text fragments
- Multiple social platform icons as abstract shapes (LinkedIn, Facebook, Reddit, X)
- A prominent glowing magnifying glass motif representing advanced search tools
- "$" symbols subtly integrated to represent opportunity and value
- Tutorial/education elements: step-by-step flow arrows, numbered steps
- Search query strings faintly visible in the background

Style:
- Modern, sleek, professional
- Search intelligence / precision sourcing aesthetic
- High contrast with the dark background
- Glowing/luminous effects on accent colors
- Conveys mastery, precision, and tool power

The image should convey: "Master the craft of precision searching across every major platform."

Dimensions: 1200x630 pixels (social media preview optimal size)
Only text allowed: "AutoNateAI" and "Advanced Search Packs + Tutorials"`
};
...
async function main() {
  console.log('🎨 AutoNateAI OG Image Generator (Shop)\n');
  ...
  if (!CONFIG.apiKey) {
    ...
    console.log('  node scripts/generate-og-shop.js\n');
    process.exit(1);
  }
